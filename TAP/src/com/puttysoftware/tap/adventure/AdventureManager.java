...
public class AdventureManager implements OpenFilesHandler {
	...
	try {
		...
		try (InputStream is = AdventureManager.class.getResourceAsStream(exName);
	    ...
	    Messager.showMessage("Loading examples failed, because a file I/O error occurred.");
	    TAP.getErrorLogger().logNonFatalError(ioe);
	}
    }

    private int getAdventureIndex() {
	String choice = CommonDialogs.showInputDialog("Which Adventure?", "Load Example Adventure",
		this.exampleFileList, this.exampleFileList[0]);
	...
	if (this.exampleFiles == null) {
	    ...
	    try (InputStream is = AdventureManager.class.getResourceAsStream(LIST_FILE);
		    ResourceStreamReader ris = new ResourceStreamReader(is)) {
		String line = "";
		while (line != null) {
		    line = ris.readString();
	    ...
	    this.exampleFiles = exampleFilesAL.toArray(new String[exampleFilesAL.size()]);
	}
	if (this.exampleFileList == null) {
	    ArrayList<String> exampleFileListAL = new ArrayList<>();
	    try (InputStream is = AdventureManager.class.getResourceAsStream(DISPLAY_LIST_FILE);
		    ResourceStreamReader ris = new ResourceStreamReader(is)) {
		String line = "";
		while (line != null) {
		    line = ris.readString();
	    ...
	    this.exampleFileList = exampleFileListAL.toArray(new String[exampleFileListAL.size()]);
	}
    }
	...
	final FileDialog fd = new FileDialog(game.getOutputFrame(), "Open Adventure", FileDialog.LOAD);
	fd.setVisible(true);
	filename = fd.getFile();
	...
	try {
	    ...
	    Messager.showMessage("Loading failed, because the file was not found?");
	    TAP.getErrorLogger().logNonFatalError(fnfe);
	    this.handleDeferredSuccess(false);
	} catch (final IOException ie) {
	    Messager.showMessage("Loading failed, because a file I/O error occurred.");
