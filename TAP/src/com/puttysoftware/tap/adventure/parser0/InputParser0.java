...
public class InputParser0 implements InputParser {
	...
	final boolean synonymCommand = this.findRoomCommand(Commands.MAIN_COMMAND_SYNONYM_TABLE);
	if (synonymCommand) {
	    final String syncmd = this.advData.get(this.counter + this.subCounter);
		...
		if (line != null) {
		    if (line.length() > 0) {
			final String[] splitLine = line.split(Commands.SYNONYM_SEPARATOR);
	...
	final boolean startCommand = this.findRoomCommand(Commands.MAIN_COMMAND_START);
	if (!startCommand) {
	...
	if (!initialJump4) {
	    Messager.showErrorMessage("The defined starting room doesn't exist!");
	    TAP.getAdventureManager().setLoaded(false);
	    return;
	}
    }
	...
	for (final String element : commandArray) {
		...
		// Found omni command
		final int cmdLen = InputParser0.omniCommandLength(preParsedCommand);
	...
	for (int z = this.counter; z < this.advData.size(); z++) {
		...
		final String[] dataSplit = data.split(Commands.RESULT_DELIMITER);
	...
	if (cmdName.length() >= 4) {
	    if (cmdName.substring(0, 4).equalsIgnoreCase(Commands.OMNI_COMMAND_SHOW)) {
		return true;
	    } else {
		return false;
	    }
	...
	try {
		...
		if (stateQuestTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_ALTER_STATE)) {
			...
			InputParser0.displayNoArgsForSpecialCommandMessage(specialCmd);
			return;
		    }
			...
			Messager.showErrorMessage("Invalid alter state arguments found!");
		    }
		} else if (stateQuestTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_CHECK_STATE)) {
			...
			final String checkAgainst = this.objectState.get(checkObjName);
			if (!checkObjState.equalsIgnoreCase(checkAgainst)) {
			    final String checkFail = this.getCommandAtOffset(2).substring(1);
			    final String delim = this.getCommandAtOffset(2).substring(0, 1);
			    if (!delim.equals(Commands.RESULT_DELIMITER)) {
				Messager.showErrorMessage("Missing = before failure result for check command!");
				return;
			    }
			    this.parseCommandResultOuter(checkFail);
			} else {
			    final String checkSuccess = this.getCommandAtOffset(1).substring(1);
			    final String delim = this.getCommandAtOffset(1).substring(0, 1);
			    if (!delim.equals(Commands.RESULT_DELIMITER)) {
				Messager.showErrorMessage("Missing = before success result for check command!");
				return;
			    }
			...
			Messager.showErrorMessage("Invalid check state arguments found!");
		    }
		} else if (stateQuestTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_QUEST)) {
			...
			if (questSubtype.equalsIgnoreCase(Commands.SPECIAL_QUEST_ARG_BEGIN)) {
			    if (!this.questState.containsKey(questName)) {
				this.questState.put(questName, Boolean.FALSE);
				this.questProgress.put(questName, Integer.valueOf(0));
			    } else {
				// Invalid quest state
				Messager.showErrorMessage("Cannot start a finished or started quest!");
			    }
			} else if (questSubtype.equalsIgnoreCase(Commands.SPECIAL_QUEST_ARG_FINISH)) {
			    if (this.questState.containsKey(questName)) {
				this.questState.put(questName, Boolean.TRUE);
				this.questProgress.put(questName, Integer.valueOf(10000));
			    } else {
				// Invalid quest state
				Messager.showErrorMessage("Cannot finish an unstarted quest!");
			    }
			} else if (questSubtype.equalsIgnoreCase(Commands.SPECIAL_QUEST_ARG_STATUS)) {
			    if (this.questState.containsKey(questName)) {
				if (this.questState.get(questName).booleanValue()) {
				    // Quest done
				    final String questDone = this.getCommandAtOffset(3).substring(1);
				    final String delim = this.getCommandAtOffset(3).substring(0, 1);
				    if (!delim.equals(Commands.RESULT_DELIMITER)) {
				    ...
				    // Quest started
				    final String questStarted = this.getCommandAtOffset(2).substring(1);
				    final String delim = this.getCommandAtOffset(2).substring(0, 1);
				    if (!delim.equals(Commands.RESULT_DELIMITER)) {
					Messager.showErrorMessage(
				...
				// Quest not started
				final String questNotStarted = this.getCommandAtOffset(1).substring(1);
				final String delim = this.getCommandAtOffset(1).substring(0, 1);
				if (!delim.equals(Commands.RESULT_DELIMITER)) {
				    Messager.showErrorMessage(
			    ...
			    Messager.showErrorMessage("Unknown arguments found for quest command: " + questSubtype);
			}
		    } else if (questArgs.length == 3) {
			final String questSubtype = questArgs[0];
			final String questName = questArgs[1];
			final int questValue = Integer.parseInt(questArgs[2]);
			if (questSubtype.equalsIgnoreCase(Commands.SPECIAL_QUEST_ARG_ADD)) {
			    if (this.questProgress.containsKey(questName)
				    && !this.questState.get(questName).booleanValue()) {
				final int newValue = this.questProgress.get(questName).intValue() + questValue;
				this.questProgress.put(questName, Integer.valueOf(newValue));
			    } else {
				// Invalid quest state
				Messager.showErrorMessage("Cannot add progress to an unstarted or finished quest!");
			    }
			} else if (questSubtype.equalsIgnoreCase(Commands.SPECIAL_QUEST_ARG_SET)) {
			    if (this.questProgress.containsKey(questName)
				    && !this.questState.get(questName).booleanValue()) {
				this.questProgress.put(questName, Integer.valueOf(questValue));
			    } else {
				// Invalid quest state
				Messager.showErrorMessage("Cannot set progress for an unstarted or finished quest!");
			    }
			} else if (questSubtype.equalsIgnoreCase(Commands.SPECIAL_QUEST_ARG_GET)) {
			    if (this.questProgress.containsKey(questName)
				    && !this.questState.get(questName).booleanValue()) {
				final int currentValue = this.questProgress.get(questName).intValue();
				if (currentValue > questValue) {
				    final String questGreater = this.getCommandAtOffset(3).substring(1);
				    final String delim = this.getCommandAtOffset(3).substring(0, 1);
				    if (!delim.equals(Commands.RESULT_DELIMITER)) {
					Messager.showErrorMessage(
				    ...
				    final String questLess = this.getCommandAtOffset(2).substring(1);
				    final String delim = this.getCommandAtOffset(2).substring(0, 1);
				    if (!delim.equals(Commands.RESULT_DELIMITER)) {
					Messager.showErrorMessage(
				    ...
				    final String questEqual = this.getCommandAtOffset(1).substring(1);
				    final String delim = this.getCommandAtOffset(1).substring(0, 1);
				    if (!delim.equals(Commands.RESULT_DELIMITER)) {
					Messager.showErrorMessage(
				...
				// Invalid quest state
				Messager.showErrorMessage("Cannot get progress for an unstarted or finished quest!");
			    }
			} else {
			    // Found unknown arguments for quest command
			    Messager.showErrorMessage("Unknown arguments found for quest command: " + questSubtype);
			...
			if (questSubtype.equalsIgnoreCase(Commands.SPECIAL_QUEST_ARG_TEST)) {
			    if (this.questProgress.containsKey(questName)
				    && !this.questState.get(questName).booleanValue()) {
				final int currentValue = this.questProgress.get(questName).intValue();
				if (currentValue > questValue1 && currentValue < questValue2) {
				    final String questBetween = this.getCommandAtOffset(1).substring(1);
				    final String delim = this.getCommandAtOffset(1).substring(0, 1);
				    if (!delim.equals(Commands.RESULT_DELIMITER)) {
					Messager.showErrorMessage(
				    ...
				    final String questOutside = this.getCommandAtOffset(3).substring(1);
				    final String delim = this.getCommandAtOffset(3).substring(0, 1);
				    if (!delim.equals(Commands.RESULT_DELIMITER)) {
					Messager.showErrorMessage(
			...
			// Found unknown arguments for quest command
			Messager.showErrorMessage("Unknown arguments found for quest command");
		    }
		} else if (warpKillTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_KILL)) {
		    ...
		    TAP.getAdventureManager().setLoaded(false);
		} else if (warpKillTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_WARP)) {
			...
			Messager.showErrorMessage("Warp attempted to nonexistent room: " + warpArg);
		    }
		} else if (specialCmdTest8.equalsIgnoreCase(Commands.SPECIAL_COMMAND_ONCE_GRAB)) {
		    String onceGrabArg = "";
		    ...
		    final String[] onceGrabArgSplit = onceGrabArg.split(Commands.HAVE_MULTIPLE);
		    boolean onceGrabbedAny = false;
		    for (int x = 0; x < onceGrabArgSplit.length; x++) {
			if (!this.inventory.contains(onceGrabArgSplit[x])
				&& !this.grabbedAlready.contains(onceGrabArgSplit[x])) {
			    onceGrabbedAny = true;
		    ...
		    if (onceGrabbedAny) {
			final String onceGrabSuccess = this.getCommandAtOffset(1);
			Messager.showMessage(onceGrabSuccess);
		    } else {
			final String onceGrabFail = this.getCommandAtOffset(2);
			Messager.showMessage(onceGrabFail);
		    }
		} else if (specialCmdTest8.equalsIgnoreCase(Commands.SPECIAL_COMMAND_ONCE_GAIN)) {
		    String onceGainArg = "";
		    if (specialCmd.length() >= 9) {
		    ...
		    final String[] onceGainArgSplit = onceGainArg.split(Commands.HAVE_MULTIPLE);
		    for (int x = 0; x < onceGainArgSplit.length; x++) {
			if (!this.inventory.contains(onceGainArgSplit[x])
				&& !this.grabbedAlready.contains(onceGainArgSplit[x])) {
			    this.inventory.add(onceGainArgSplit[x]);
			    this.grabbedAlready.add(onceGainArgSplit[x]);
			}
		    }
		} else if (warpKillTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_GAIN)) {
		    String gainArg = "";
		    if (specialCmd.length() >= 5) {
		    ...
		    final String[] gainArgSplit = gainArg.split(Commands.HAVE_MULTIPLE);
		...
		} else if (warpKillTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_GRAB)) {
		    String grabArg = "";
		    if (specialCmd.length() >= 5) {
		    ...
		    final String[] grabArgSplit = grabArg.split(Commands.HAVE_MULTIPLE);
		...
		} else if (warpKillTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_HAVE)) {
		    String haveArg = "";
		    if (specialCmd.length() >= 5) {
		    ...
		    final String[] haveArgSplit = haveArg.split(Commands.HAVE_MULTIPLE);
		    boolean haveIt = true;
		    for (final String element : haveArgSplit) {
			haveIt = haveIt && this.inventory.contains(element);
		    }
		    if (!haveIt) {
			final String haveFail = this.getCommandAtOffset(2).substring(1);
			final String delim = this.getCommandAtOffset(2).substring(0, 1);
			if (!delim.equals(Commands.RESULT_DELIMITER)) {
			    Messager.showErrorMessage("Missing = before failure result for have command!");
			    return;
			}
			this.parseCommandResultOuter(haveFail);
		    } else {
			final String haveSuccess = this.getCommandAtOffset(1).substring(1);
			final String delim = this.getCommandAtOffset(1).substring(0, 1);
			if (!delim.equals(Commands.RESULT_DELIMITER)) {
			    Messager.showErrorMessage("Missing = before success result for have command!");
			    return;
			}
			this.parseCommandResultOuter(haveSuccess);
		    }
		} else if (warpKillTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_DROP)) {
		    String dropArg = "";
		    if (specialCmd.length() >= 5) {
		    ...
		    final String[] dropArgSplit = dropArg.split(Commands.HAVE_MULTIPLE);
		    boolean dropIt = true;
		...
		} else if (warpKillTest.equalsIgnoreCase(Commands.SPECIAL_COMMAND_LOSE)) {
		    String loseArg = "";
		    if (specialCmd.length() >= 5) {
		    ...
		    final String[] loseArgSplit = loseArg.split(Commands.HAVE_MULTIPLE);
		    boolean loseIt = true;
		    ...
		    Messager.showErrorMessage("Unknown special command found: " + specialCmd);
		}
	    } else {
		// Text to output
	...
	try {
		...
		if (cmdArg.equalsIgnoreCase(Commands.OMNI_SHOW_ARG_ITEMS)
			|| cmdArg.equalsIgnoreCase(Commands.OMNI_SHOW_ARG_INVENTORY)) {
			...
			|| cmdArg.equalsIgnoreCase(Commands.OMNI_SHOW_ARG_FORM)) {
		    if (this.inventory.isEmpty()) {
			Messager.showMessage("You have nothing.");
		    } else {
			Messager.showMessage("You have:");
		...
		} else if (cmdArg.equalsIgnoreCase(Commands.OMNI_SHOW_ARG_QUEST_DETAILS)) {
		    if (this.questProgress.isEmpty()) {
			Messager.showMessage("You have no quests.");
		    } else {
			Messager.showMessage("You have the following quests:");
			final Enumeration<String> quests = this.questProgress.keys();
			while (quests.hasMoreElements()) {
			    final String quest = quests.nextElement();
			    final int progress = this.questProgress.get(quest).intValue();
			    final String questPercentFirst = Integer.toString(progress / 100);
			    String questPercentSecond = Integer.toString(progress % 100);
			    if (progress % 100 < 10) {
				questPercentSecond = "0" + questPercentSecond;
			    }
			    final String questPercent = questPercentFirst + "." + questPercentSecond + "%";
			    Messager.showMessage(quest + ": " + questPercent + " Completed");
			}
		    }
		} else {
		    // Found unknown argument for show omni-command
		    Messager.showErrorMessage("Unknown argument for show command found: " + cmdArg);
		}
	    }
	} catch (final RuntimeException re) {
	    // Parse error
	...
	Messager.showErrorMessage("Malformed special command found: " + cmd + ".");
    }

    private static void displayNoArgsForSpecialCommandMessage(final String cmd) {
	Messager.showErrorMessage(
		"No arguments were specified for special command: " + cmd + ", and arguments are required!");
    }

    private static void displayParsingErrorMessage(final String cmd) {
	Messager.showErrorMessage("A parsing error occurred while trying to parse " + cmd + ".");
    }
}
